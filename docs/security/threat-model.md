...
| Mandate/receipt tampering     | pinned Ed25519 keys in both directions, canonical JSON, exact scope/time checks, atomic nonce and receipt replay protection, SQL tasks | managed rotation and protocol audit remain                     |
...
| Forged operator mutation      | signed HttpOnly session, matching CSRF header, exact credentialed CORS, Secure cross-site hosted cookie, non-default-secret guard      | shared operator is not identity or tenant authorization        |
...
`A2A_ENABLED=false`, `CUSTOMER_AGENT_TASK_STORE=memory`,
`CUSTOMER_AGENT_REAL_SIGNING_ENABLED=false`, and mock receipt signing/verification are the safe
defaults. Hosted A2A fails readiness or receipt completion unless SQL storage and the configured,
pinned signing pair are available. The
...
The hosted demo has a signed server-side operator session, matching CSRF token, an exact frontend
origin, a `Secure; SameSite=None` cookie for the split web/API origins, signed A2A receipts, and
durable SQL customer-agent tasks whose readiness is checked before promotion. Those controls close
the anonymous demo-action, forged-receipt, and restart-loss gaps; they are not production identity.
